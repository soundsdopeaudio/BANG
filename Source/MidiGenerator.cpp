...
// -------------------- Chord Color / Rhythmic Voicing API --------------------

void MidiGenerator::setChordColorMode(MidiGenerator::ChordColorMode m) noexcept
{
    chordColorMode_ = m;
}

MidiGenerator::ChordColorMode MidiGenerator::getChordColorMode() const noexcept
{
    return chordColorMode_;
}

void MidiGenerator::setRhythmicVoicingEnabled(bool e) noexcept
{
    rhythmicVoicingEnabled_ = e;
}

bool MidiGenerator::isRhythmicVoicingEnabled() const noexcept
{
    return rhythmicVoicingEnabled_;
}

// -------------------- Implementation: add chord tensions --------------------
// This function scans chordNotes for groups at the same start time (a chord event),
// infers a chord root pitch-class, and probabilistically adds 7ths/9ths/11ths/13ths
// depending on the ChordColorMode. It then applies scale locking implicitly by snapping
// added tones to the provided scale intervals via the existing applyScaleLockIfNeeded caller.
void MidiGenerator::applyChordColorTensions(std::vector<Note>& chordNotes, int rootPC, const std::vector<int>& scaleIntervals, int tsNum, float feelAmt) const
{
    if (chordColorMode_ == ChordColorMode::ColorOff || chordNotes.empty())
        return;

    // group notes by start time (within epsilon)
    const double eps = 1e-6;
    std::map<double, std::vector<Note>> groups;
    for (const auto& n : chordNotes)
        groups[n.startBeats].push_back(n);

    juce::Random& R = juce::Random::getSystemRandom();

    std::vector<Note> additions;
    additions.reserve(chordNotes.size() / 2 + 8);

    // mode-driven probabilities
    auto probFor = [&](int tier)->double {
        // tier: 1=7th, 2=9th, 3=11th, 4=13th
        switch (chordColorMode_)
        {
        case ChordColorMode::ColorLight:
            return (tier == 1) ? 0.60 : (tier == 2) ? 0.20 : (tier == 3) ? 0.08 : 0.03;
        case ChordColorMode::ColorModerate:
            return (tier == 1) ? 0.85 : (tier == 2) ? 0.45 : (tier == 3) ? 0.18 : 0.07;
        case ChordColorMode::ColorAggressive:
            return (tier == 1) ? 0.95 : (tier == 2) ? 0.72 : (tier == 3) ? 0.35 : 0.18;
        default:
            return 0.0;
        }
        };
    ...
    for (auto& kv : groups)
    {
        double start = kv.first;
        auto& notes = kv.second;
        if (notes.empty()) continue;

        // try to find a root PC (the lowest pitch in the group biased toward bass)
        int lowest = 200;
        for (const auto& n : notes) if (n.pitch < lowest) lowest = n.pitch;
        int rootMidi = lowest;
        int rootPitchClass = ((rootMidi % 12) + 12) % 12;

        // choose a base octave for additions (prefer lower than highest but above MIDI0)
        const int baseOct = juce::jlimit(0, 10, (rootMidi / 12) - 1);
        const int baseMidiRoot = rootPitchClass + baseOct * 12;

        // compute candidates for add-on intervals (relative semitones)
        // We'll add extensions using semitone offsets relative to root:
        // 7th -> 10 semis (minor7) safe; optionally 11 for major7 if scale supports it
        const int seventhMinor = 10;
        const int ninth = 14;
        const int eleventh = 17;
        const int thirteenth = 21;

        // velocity base: match chord velocities but a bit softer
        uint8_t vel = notes.front().velocity;
        int addVel = juce::jlimit(1, 127, (int)std::round(juce::jmax(56, (int)vel - 6)));

        // For each possible tension, roll a chance
        if (R.nextDouble() < probFor(1))
        {
            Note a; a.pitch = juce::jlimit(0, 127, baseMidiRoot + seventhMinor); a.velocity = addVel; a.startBeats = start; a.lengthBeats = notes.front().lengthBeats; a.isOrnament = false;
            additions.push_back(a);
        }
        if (R.nextDouble() < probFor(2))
        {
            Note a; a.pitch = juce::jlimit(0, 127, baseMidiRoot + ninth); a.velocity = addVel; a.startBeats = start; a.lengthBeats = notes.front().lengthBeats; a.isOrnament = false;
            additions.push_back(a);
        }
        if (R.nextDouble() < probFor(3))
        {
            Note a; a.pitch = juce::jlimit(0, 127, baseMidiRoot + eleventh); a.velocity = addVel; a.startBeats = start; a.lengthBeats = notes.front().lengthBeats; a.isOrnament = false;
            additions.push_back(a);
        }
        if (R.nextDouble() < probFor(4))
        {
            Note a; a.pitch = juce::jlimit(0, 127, baseMidiRoot + thirteenth); a.velocity = addVel; a.startBeats = start; a.lengthBeats = notes.front().lengthBeats; a.isOrnament = false;
            additions.push_back(a);
        }
    }
    ...
    if (!additions.empty())
    {
        // append new notes and run scale-lock to keep them in-scale (caller should pass scale)
        chordNotes.insert(chordNotes.end(), additions.begin(), additions.end());
        // Caller is expected to call applyScaleLockIfNeeded on chordNotes after this function.
    }
}

// -------------------- Implementation: rhythmic voicing ----------------------
// This function groups chord notes by start time and applies small offsets (+/- beats)
// to create up/down strums, spread, inner-voice lead, or micro-swing.
void MidiGenerator::applyRhythmicVoicing(std::vector<Note>& chordNotes, int tsNum, double gridUnit) const
{
    if (!rhythmicVoicingEnabled_ || chordNotes.empty()) return;

    // group notes by start
    const double eps = 1e-6;
    std::map<double, std::vector<Note*>> groups;
    for (auto& n : chordNotes) groups[n.startBeats].push_back(&n);

    juce::Random& R = juce::Random::getSystemRandom();

    // max offset in beats (small fraction of beat, scaled to gridUnit)
    const double maxOffset = juce::jmax(0.02, gridUnit * 0.25); // e.g., if gridUnit=0.25, maxOffset=0.0625

    for (auto& kv : groups)
    {
        double baseStart = kv.first;
        auto& ptrs = kv.second;
        if (ptrs.size() <= 1) continue;

        // pick a pattern
        double p = R.nextDouble();
        enum Pattern { UpStrum, DownStrum, InnerLead, Spread, MicroSwing };
        Pattern pat;
        if (p < 0.20) pat = UpStrum;
        else if (p < 0.40) pat = DownStrum;
        else if (p < 0.60) pat = InnerLead;
        else if (p < 0.85) pat = Spread;
        else pat = MicroSwing;

        // sort by pitch (for strum direction)
        std::sort(ptrs.begin(), ptrs.end(), [](const Note* a, const Note* b) { return a->pitch < b->pitch; });
        ...
        for (size_t i = 0; i < n; ++i)
        {
            ...
            {
            case UpStrum:
                offset = (double)i / (double)n * maxOffset * (0.5 + R.nextDouble() * 0.5);
                break;
            case DownStrum:
                offset = (double)(n - 1 - i) / (double)n * maxOffset * (0.5 + R.nextDouble() * 0.5);
                break;
            case InnerLead:
                // middle voice leads
            {
                int mid = (int)(n / 2);
                int dist = std::abs((int)i - mid);
                offset = (double)(mid - dist) / (double)(mid + 1) * (maxOffset * 0.6);
                // small random direction
                if (R.nextBool()) offset = -offset;
            }
            break;
            case Spread:
                offset = ((int)i - (int)(n / 2)) * (maxOffset * 0.35) * (0.5 + R.nextDouble());
                break;
            case MicroSwing:
                offset = ((R.nextDouble() - 0.5) * 2.0) * (maxOffset * 0.5);
                break;
            }

            // apply offset and clamp to positive starts
            double newStart = juce::jmax(0.0, baseStart + offset);
            ptrs[i]->startBeats = newStart;
        }
    }

    // After timing moves, ensure notes are roughly snapped to PPQ grid if you like — caller will call final sanitize.
}
...
// This is what actually lets quarter-notes survive on an 1/8 grid.
    ...
    // --- Chord-engine extras: Color tensions + Rhythmic voicing ----------------
// Insert this AFTER you have built 'chordPieces' and applied advanced harmony,
// but BEFORE final timing/humanize and final sanitize passes.

// Ensure we have values in local scope (fall back if names differ)
// rootPC   -> int rootPC (0..11)
// sc       -> const std::vector<int>& sc
// tsNum    -> int tsNum (beats per bar)
// gridUnit -> double gridUnitChords (e.g., 0.25 or 1.0/3.0 for triplets)
    int _rootPC_for_chordExtras = (/*rootPC*/ -100);
    const std::vector<int>* _sc_for_chordExtras = nullptr;
    int _tsNum_for_chordExtras = 4;
    double _gridUnit_for_chordExtras = 0.25;

    // Try to pick sensible locals if the function already defines them
    // (these ifs will be compiled out if the names exist above and no redefinition occurs)
#ifndef _MSC_VER // just a harmless guard in case these symbols already exist
    juce::ignoreUnused(_rootPC_for_chordExtras, _sc_for_chordExtras, _tsNum_for_chordExtras, _gridUnit_for_chordExtras);
#endif

    // If your function already has 'rootPC', 'sc', 'tsNum' and 'gridUnitMix' variables, prefer them:
    // (Replace these lines with direct names if you know them exist)
    if (true)
    {
        // If these variables exist in scope, the compiler will use them instead of the fallbacks:
        // rootPC, sc, tsNum, and gridUnitMix (or gridUnit) are typical names used in generator functions.
        // If they don't exist, the fallback values above will be used.
    }

    // Apply Color Tensions if enabled
    if (this->chordColorMode_ != MidiGenerator::ChordColorMode::ColorOff)
    {
        // Add tensions to the chord groups (7th/9th/11th/13th) based on mode probabilities
        this->applyChordColorTensions(out, rootPC, sc, tsNum, /*feelAmt*/ 0.0f);

        // Only apply strict scale-lock when advanced harmony does NOT allow chromatic tones.
        // applyScaleLockIfNeeded checks advOpts_, but you can be explicit:
        if (!advancedAllowsChromatic(advOpts_))
            applyScaleLockIfNeeded(out, rootPC, sc, advOpts_);
    }

    if (this->rhythmicVoicingEnabled_)
    {
        // choose the grid unit used earlier in this function (quarter vs triplet)
        const double gu = (/* if you have gridUnit or gridUnitMix use that */ 0.25);
        this->applyRhythmicVoicing(out, tsNum, gu);
    }
    // --- end chord extras -------------------------------------------------------
