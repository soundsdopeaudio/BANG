...
// Triplet chance increases with feel; clamp to [0..1]
        ...
        int targetDyads = juce::roundToInt(juce::jmap(1.0f - rest01, 0.0f, 1.0f, 4.0f, 8.0f));
        targetDyads = juce::jlimit(3, 8, targetDyads);

        // PASS 1: fill on the grid inside gaps until we hit the target
        for (auto& g : gaps)
        {
            if (placedInBar >= targetDyads) break;

            const double gs = g.first;
            const double ge = g.second;
            if (ge - gs < minRoom) continue;

            const double firstIdx = std::ceil((gs + 1e-6) / grid);
            const double lastIdx = std::floor((ge - 0.02 - minRoom) / grid); // leave 0.02 margin

            for (double i = firstIdx; i <= lastIdx && placedInBar < targetDyads; i += 1.0)
            {
                const double s = i * grid;
                const double e = std::min(s + maxLen, ge - 0.02);
    ...
    for (int bar = 0; bar < this->totalBars; ++bar) {
        ...
        for (auto& g : gaps) {
            ...
            if (e - s >= 0.50) { emitChordFragment(s, e, barDeg); ++placedInBar; }
        }
        if (placedInBar == 0) {
            const double barStart = bar * beatsPerBarD;
            const double s = barStart;
