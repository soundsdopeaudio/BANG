...
static const std::vector<std::pair<const char*, std::vector<int>>> kScales = {
...
const Scale& MidiGenerator::getScaleByIndex(int idx) const
{
    if (idx < 0) idx = 0;
    if (idx >= static_cast<int>(kScales.size()))
        idx = static_cast<int>(kScales.size()) - 1;
    return kScales[(size_t)idx];
}
