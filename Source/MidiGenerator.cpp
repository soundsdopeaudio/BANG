...
std::vector<Note> MidiGenerator::generateMelody()
{
    ...
    // --- Pick a predominant note length mode for this generation (random 1 of 4) ---
    {
        ...
        const int pick = sysRng.nextInt({ 0, 4 }); // 0..3
        switch (pick)
        {
...
MidiGenerator::MixBundle MidiGenerator::generateMelodyAndChords(bool avoidOverlaps)
{
    ...
    auto emitDyad = [&](double s, double e, int barDeg) -> void
    {
        ...
        // Compute the scale degree -> pitch class for this bar (no extra helpers)
        const auto& sc = this->currentScale;
        const int   rootPC = this->keySemitone_;
        if (sc.empty()) return;

        int idx = barDeg % (int)sc.size();
        if (idx < 0) idx += (int)sc.size();
        const int pcA = (rootPC + sc[(size_t)idx]) % 12;   // dyad root PC for this bar
...
// Iterate bars, compute gaps, place dyads (respect rest density; guarantee ≥ 1 per bar)
    for (int bar = 0; bar < this->totalBars; ++bar)
    {
        ...
        // simple: use 8th grid, higher target = more chord pieces
// Triplet chance increases with feel; clamp to [0..1]
        ...
        // Grid & room
        const double grid = mixTrip ? (1.0 / 3.0) : 0.5;  // 8th-triplet vs straight 8th
        const double maxLen = mixTrip ? (2.0 / 3.0) : 0.50; // typical dyad length
        const double minRoom = mixTrip ? (1.0 / 6.0) : 0.20; // allow short stabs

        // Target dyads per bar: rest low→more, rest high→fewer (but still active)
        int targetDyads = juce::roundToInt(juce::jmap(1.0f - rest01, 0.0f, 1.0f, 2.0f, 4.0f));
        targetDyads = juce::jlimit(1, 4, targetDyads);

        // PASS 1: fill on the grid inside gaps until we hit the target
        for (auto& g : gaps)
        {
            const double gs = (double)g.first;
            const double ge = (double)g.second;

            // --- phase-aware candidate walk (keeps triplet vs straight grid alignment) ---
            const bool useTrip = (density == Density::Triplet8th); // keep your density flag
            const int phaseSteps = useTrip ? 3 : 2;

            auto& sysRng = juce::Random::getSystemRandom();
            const int phaseIdx = sysRng.nextInt(phaseSteps);   // 0..phaseSteps-1
            const double phase = (phaseIdx / (double)phaseSteps) * grid;

            // aligned index range inside this gap, shifted by phase
        ...
        // PASS 2: ensure a lively minimum if gaps were sparse (try edges of the biggest gaps)
        if (placedInBar < 3)
        {
            // Find the two widest gaps in this bar
    ...
    auto thinChordPieces = [&](std::vector<Note>& notes)
        {
            ...
            for (const auto& n : notes)
            {
                ...
                // Chords: favor keeping *both* downbeats and half-beats more than melody
...
// Scale lock if Advanced Harmony isn’t allowing chromatic
    applyScaleLockIfNeeded(melody, rootPC, sc, advOpts_);
    applyScaleLockIfNeeded(chordPieces, rootPC, sc, advOpts_);

    // Use density-aware grid for timing/sanitize (triplets get 1/3)
    ...
    bang_applyTimingAndEnds(chordPieces, tsNum, swingAmtLocal, timeHuman, velHuman, gridUnitMix);
