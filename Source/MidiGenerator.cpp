...
//      so stacked tones remain phase-aligned.
    {
        ...
        for (auto& kv : groups)
        {
            ...
            for (size_t i : idxs)
            {
                ...
                if (offbeat8th)
                    notes[i].startBeats += swingAmt * 0.5; // triplet-lean
            }
        }
    }
...
// Shuffle once to break ties, then sort by score (deterministic comparator)
    ...
    std::vector<Note> chords; chords.reserve(48);
    ...
    applyAdvancedHarmonyToChordNotes(chords); // your existing hook mutates chord notes as needed

    // ===== 7) (Optional) thin dyads if too dense for Half density path =====
    if (density == Density::Half) {
        // keep only first dyad per bar
        std::vector<Note> thinned; thinned.reserve(chords.size());
        for (int bar = 0; bar < this->totalBars; ++bar) {
...

    {
        ...
        // Ensure at least one melody note survives per bar (so phrases don’t vanish)
    ...
    if (!melody.empty() && feelAmt > 0.0f)
    {
        for (auto& n : melody)
        {
            ...
            // Shorten offbeats a hair, leave downbeats alone. Max trim ≈ 12.5% at feel=1
            const double trim = onBeat ? 0.0
                : (halfBeat ? 0.10 : 0.125) * (double)feelAmt;
    ...
    if (!chordPieces.empty() && feelAmt > 0.0f)
    {
        for (auto& n : chordPieces)
        {
            ...
            const double trim = strong ? 0.05 * (double)feelAmt : 0.12 * (double)feelAmt;
            n.lengthBeats = std::max(0.125, n.lengthBeats * (1.0 - trim));
        }
    }
    ...
    bang_finalSanitizeNotes(chords, 0.25, /*monophonic*/ false);
