...
// Clamp timing offset range (beats)
    ...
    for (size_t i = 0; i + 1 < notes.size(); ++i)
    {
        ...
        if (aEnd > b.startBeats)
        {
            ...
            a.lengthBeats = std::max(0.05, desiredEnd - a.startBeats);
        }
    }
...
static void bang_limitChordSubdivisionTo8ths(std::vector<Note>& chords, double beatsPerBar)
{
    ...
    for (size_t i = 0; i < chords.size(); ++i)
    {
        ...
        // Avoid overlap into next chord with same pitch (simple hygiene)
        if (i + 1 < chords.size())
        {
            double end = n.startBeats + n.lengthBeats;
            if (end > chords[i + 1].startBeats)
                n.lengthBeats = std::max(0.05, chords[i + 1].startBeats - n.startBeats);
        }
    }
}
...
static void bang_limitMelodyFastNotes(std::vector<Note>& melody)
{
    if (melody.empty()) return;

    // Decide if this batch is allowed to keep fast notes
    bool allowFastNow = false;
    if (g_bangMelodyNoteCounter % 100 == 0)
        allowFastNow = (juce::Random::getSystemRandom().nextBool()); // 50%

    const double minLen = 0.5; // 1/8 beat
    for (auto& n : melody)
    {
        ++g_bangMelodyNoteCounter;
        ...
        {
            // Even when allowed, clamp *extreme* 1/32 to at least 1/16 (optional safety)
            if (n.lengthBeats < 0.25) // 1/16
                n.lengthBeats = 0.25;
        }
    }
}
...
static void bang_enforceMonophonic(std::vector<Note>& notes)
{
    ...
    for (size_t k = 0; k + 1 < collapsed.size(); ++k)
    {
        ...
        if (aEnd > b.startBeats - minGap)
            a.lengthBeats = std::max(0.05, (b.startBeats - minGap) - a.startBeats);
    }
...
std::vector<Note> MidiGenerator::generateMelody()
{
    ...
    // --- Phrase shaping & loop logic (with 5% skip)
    {
        ...
        bang_limitMelodyFastNotes(out);
    }

    // --- Enforce monophony before humanize
    //bang_enforceMonophonic(out);

    // --- Humanization clamps + end-snap (ALWAYS last)
    {
       // bang_applyTimingAndEnds(out, beatsPerBar,
         //   getSwingAmount(),
         //   getHumanizeTiming(),
          //  getHumanizeVelocity(),
          //  0.50 /* eighth note */);
    ...
    // --- FINAL GUARD: remove any blips & enforce >= 1/8 (monophonic melody)
   // bang_finalSanitizeNotes(out, 0.50 /* 1/8 beat */, true /* monophonic */);
...
MidiGenerator::MixBundle MidiGenerator::generateMelodyAndChords(bool avoidOverlaps)
{
    ...
    // 2) Build an interleaved “Mixture” timeline: chord fragments + single melody notes
    std::vector<Note> chordPieces;
    chordPieces.reserve(chords.size()); // rough

    // --- helpers to read current musical grid ---
    const int bars = getBars();
    const int tsNum = getTSNumerator();
    const int tsDen = getTSDenominator(); juce::ignoreUnused(tsDen);
    const double totalBeats = (double)bars * (double)tsNum;

    // We’ll walk forward in time and alternate between adding a chord fragment and a single melody note.
    double t = 0.0;
    int notesPlaced = 0;


    // Duration chooser:
    // - Mostly 1/8 (0.5 beats) and 1/4 (1.0 beats), with a rare dotted 1/4 (1.5).
    // - Every ~20th–30th "note slot", 50% chance to inject a 1/16 (0.25) or 1/32 (0.125).
    // - Every 50th "note slot", 50% chance to inject a 1/2 note (2.0 beats).
    auto chooseLen = [this, &notesPlaced]() -> double
    {
        // 50% chance of a 1/2 note exactly every 50th slot.
        if (notesPlaced > 0 && (notesPlaced % 50 == 0) && rand01() < 0.5)
            return 2.0; // half note (2 beats assuming quarter=1 beat)

        // 20th–30th window injection: repeat every 20 slots, window ~10 slots, 50% chance
        const int mod20 = (notesPlaced % 20);
        const bool inInjectionWindow = (mod20 == 0 || (mod20 >= 1 && mod20 <= 10));
        if (inInjectionWindow && rand01() < 0.5)
        {
            // 1/16 or 1/32
            return (rand01() < 0.66) ? 0.25 : 0.125;
        }

        // Base distribution: prefer 1/8, then 1/4, with a rare dotted 1/4
        const double r = rand01();
        if (r < 0.55) return 0.5;  // 1/8
        if (r < 0.95) return 1.0;  // 1/4
        return 1.5;                // dotted 1/4 (groove spice)
    };

    // Snap a time window [t, t+len) to collect relevant notes from an existing track
    auto collectWindowNotes = [](const std::vector<Note>& src, double startBeat, double lenBeats) -> std::vector<Note>
    {
        const double endBeat = startBeat + juce::jmax(0.05, lenBeats);
        std::vector<Note> out;
        out.reserve(8);
        for (const auto& n : src)
        {
            const double n1 = n.startBeats;
            const double n2 = n.startBeats + n.lengthBeats;
            if (n1 < endBeat && startBeat < n2) // intersects window
                out.push_back(n);
        }
        return out;
    };

    // Choose a random subset of chord tones (2 or 3) that *start* inside [t, t+len)
    auto buildChordFragment = [this](const std::vector<Note>& window, double t, double len) -> std::vector<Note>
    {
        std::vector<Note> out;
        out.reserve(3);

        // Filter to notes that begin inside this window (keeps comp tight)
        std::vector<const Note*> starts;
        starts.reserve(window.size());
        for (const auto& n : window)
            if (n.startBeats >= t && n.startBeats < t + len)
                starts.push_back(&n);

        if (starts.empty())
            return out;

        // Randomly pick 2 or 3 simultaneous tones; trim to the window length
        const int howMany = (starts.size() >= 3 ? ((rand01() < 0.6) ? 3 : 2) : juce::jmax(1, (int)starts.size()));
        std::vector<int> idx(starts.size()); std::iota(idx.begin(), idx.end(), 0);
        std::shuffle(idx.begin(), idx.end(), rng);
        for (int i = 0; i < howMany; ++i)
        {
            const Note& src = *starts[(size_t)idx[(size_t)i]];
            Note n = src;
            n.startBeats = t;
            n.lengthBeats = juce::jmax(0.05, juce::jmin(src.startBeats + src.lengthBeats, t + len) - t);
    ...
    // Choose one single melody note that intersects the window; force it to live *inside* the window
    auto pickSingleMelodyNote = [](const std::vector<Note>& window, double t, double len) -> std::optional<Note>
    {
        if (window.empty()) return std::nullopt;

        const double endBeat = t + len;
        const Note* best = nullptr;
        double bestLen = 0.0;
        for (const auto& n : window)
        {
            const double s = juce::jmax(t, n.startBeats);
            const double e = juce::jmin(endBeat, n.startBeats + n.lengthBeats);
            const double interLen = juce::jmax(0.0, e - s);
            if (interLen > bestLen) { bestLen = interLen; best = &n; }
        }
        if (!best) return std::nullopt;

        Note out = *best;
        out.startBeats = t;
        out.lengthBeats = juce::jmax(0.05, juce::jmin(best->startBeats + best->lengthBeats, endBeat) - t);
        out.isOrnament = false; // ensure it’s a clear single tone
        return out;
    };

    // Walk the grid and alternate: CHORD FRAGMENT -> MELODY SINGLE -> CHORD FRAGMENT -> ...
    bool doChordFrag = true; // start with a chord fragment
    std::vector<Note> interleavedMelody; interleavedMelody.reserve(melody.size());

    while (t < totalBeats)
    {
        const double len = chooseLen();

        if (doChordFrag)
        {
            // chord fragment window
            auto w = collectWindowNotes(chords, t, len);
            auto frag = buildChordFragment(w, t, len);
            // Only push if we actually found at least 2 tones; otherwise fallback to melody singleton
            if (frag.size() >= 2)
            {
                chordPieces.insert(chordPieces.end(), frag.begin(), frag.end());
            }
            else
            {
                auto mw = collectWindowNotes(melody, t, len);
                if (auto one = pickSingleMelodyNote(mw, t, len))
                    interleavedMelody.push_back(*one);
            }
        }
        else
        {
            ...
            {
                // if melody window empty, drop a simple chord fragment so timeline stays filled
                auto w = collectWindowNotes(chords, t, len);
                auto frag = buildChordFragment(w, t, len);
                chordPieces.insert(chordPieces.end(), frag.begin(), frag.end());
            }
        ...
        ++notesPlaced;
        doChordFrag = !doChordFrag;
        t += len;
    }

    // Overwrite melody with our interleaved single-note line
    melody.swap(interleavedMelody);
    ...
    // --- HARD SCALE LOCK (local, no globals, no new types) ---
    {
        const int rootPC = getKeySemitone();
        const auto& sc = getScaleByIndex(getScaleIndex()).intervals;
        ...
        auto snapPc = [&](int midi) -> int
        {
            ...
            // keep the original octave, swap just the pitch-class
            const int base = in - pc;
            return juce::jlimit(0, 127, base + bestPc);
        };

        auto lockVec = [&](std::vector<Note>& notes)
        {
            for (auto& n : notes)
                n.pitch = snapPc(n.pitch);
        };
        ...
        lockVec(out.melody);
        lockVec(out.chords);
    } // <-- HARD SCALE LOCK block ends here

    // --- Enforce monophony (after scale-lock, before downbeat snap & humanize)
    //bang_enforceMonophonic(out.melody);

    // --- Patch B-2: Enforce chord-tone at every bar downbeat for the melody
    {
        const int bars = getBars();
        const int beatsPerBar = getTSNumerator();
        if (bars > 0 && beatsPerBar > 0)
        {
            std::vector<int> barRootMidi(bars, -1);
            std::vector<std::array<int, 3>> barTriadClass(bars, { 0,4,7 });
            std::vector<std::vector<int>> barPitchClasses(bars);

            for (const auto& cn : out.chords)
            {
                if (cn.lengthBeats <= 0.0) continue;
                if (cn.startBeats < 0.0)    continue;
                const int barIdx = (int)std::floor(cn.startBeats / (double)beatsPerBar);
                if (barIdx < 0 || barIdx >= bars) continue;

                if (barRootMidi[barIdx] < 0 || cn.pitch < barRootMidi[barIdx])
                    barRootMidi[barIdx] = cn.pitch;

                int pc = cn.pitch % 12; if (pc < 0) pc += 12;
                barPitchClasses[barIdx].push_back(pc);
            }

            for (int b = 0; b < bars; ++b)
            {
                if (barRootMidi[b] < 0) continue;
                std::sort(barPitchClasses[b].begin(), barPitchClasses[b].end());
                barPitchClasses[b].erase(std::unique(barPitchClasses[b].begin(), barPitchClasses[b].end()),
                    barPitchClasses[b].end());

                const int rootPc = (barRootMidi[b] % 12 + 12) % 12;
                const auto hasPc = [&](int rel)->bool {
                    const int want = (rootPc + rel) % 12;
                    return std::binary_search(barPitchClasses[b].begin(), barPitchClasses[b].end(), want);
                };

                if (hasPc(4))      barTriadClass[b] = { 0,4,7 };  // major
                else if (hasPc(3)) barTriadClass[b] = { 0,3,7 };  // minor
                else               barTriadClass[b] = { 0,4,7 };
            }

            for (auto& n : out.melody)
            {
                const double eps = 1e-6;
                const int    barIx = (int)std::floor(n.startBeats / (double)beatsPerBar);
                if (barIx < 0 || barIx >= bars) continue;

                const double modB = std::fmod(n.startBeats + eps, (double)beatsPerBar);
                if (modB > eps) continue; // not a bar downbeat

                const int rootMidi = barRootMidi[barIx];
                if (rootMidi < 0) continue;

                const auto triadClass = barTriadClass[barIx];
                if (!bang_isChordToneMidi(n.pitch, rootMidi, triadClass))
                    n.pitch = bang_snapToNearestChordTone(n.pitch, rootMidi, triadClass);
            }
        }
    }


    // --- FINAL GUARD: remove blips & enforce >= 1/8
   // bang_finalSanitizeNotes(out.melody, 0.50 /* 1/8 beat */, true  /* monophonic */);
   // bang_finalSanitizeNotes(out.chords, 0.50 /* 1/8 beat */, false /* polyphonic */);
