...
static inline int applyGlobalOctaveShift(int midiNote, const BANGAudioProcessor& processor)
{
    int shifted = midiNote + processor.getOctaveShiftSemitones();
...
std::vector<Note> MidiGenerator::generateChords()
{
    ...
    // ===== Global rhythm segmenter (exactly what you liked) =====
    std::vector<double> segPalette { 2.0, 1.0, 0.5, 0.5, 0.25 };
    ...
    {
        if (v >= 1.99 && v <= 2.01) return 0.55f; // half
        if (v >= 0.99 && v <= 1.01) return 1.00f; // quarter
        if (v >= 0.49 && v <= 0.51) return 0.85f; // eighth
        return 0.30f;                              // 16th
    };

    auto chooseSegLen = [&](double remaining)->double
    {
        ...
        float sum = 0.f; for (double v : c) sum += segWeight(v);
        float t = (float)rand01() * sum;
        for (double v : c) { if ((t -= segWeight(v)) <= 0.f) return v; }
        return c.front();
    };

    // ===== Progression expansion logic (mutators + Markov-ish walker) =====
    // 1) Start from a chosen seed.
    const Prog& seed = chooseProg();
    std::vector<int> base = seed.deg;
    ...
    while (songPosBeats < songBeats)
    {
        ...
        {
            double segLen = chooseSegLen(remaining);

            // merge tiny segments to avoid over-choppiness
            if (segLen <= 0.5 && remaining - segLen >= 0.25 && rand01() < 0.20)
            {
                double seg2 = chooseSegLen(remaining - segLen);
                if (seg2 <= 0.5) segLen += seg2;
            }
    ...
    for (const auto& cs : schedule)
    {
        ...
        pushNote(p0, start, len, baseVel);
        pushNote(p1, start, len, baseVel - 5);
        pushNote(p2, start, len, baseVel - 8);

        beatCursor += cs.dur;
    }

    // Advanced harmony post-process hook (no-ops if adv opts not set)
    applyAdvancedHarmonyToChordNotes(out);
