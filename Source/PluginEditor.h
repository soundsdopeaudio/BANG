...
private:
    ...
    int currentEngineIndex = 2; // 0=chords, 1=mixture (default), 2=melody
    ...
    // --- Call & Response labels & toggles
    juce::ImageComponent crLbl;
    juce::ImageButton  crToggle;
    // --- Motif labels & toggles
    juce::ImageComponent motifLbl;
    juce::ImageButton  motifToggle;    // Motif Mode toggle (image-based)

    // --- Harmony Stacks labels & combo box
    juce::ImageComponent stacksLbl;   // stacksLbl.png
    juce::ComboBox        stacksBox;  // OFF / 3 / 6 / O5 / SP

    // ---  Counter labels & toggles (right-side harmony row)
    juce::ImageComponent counterLbl;    // counterLbl.png
    juce::ImageButton  counterToggle;  // Countermelody toggle (image-based)

    // Chord engine UI: Color tensions combo + rhythmic voicing toggle
    juce::ComboBox chordColorBox;     // OFF / Light / Moderate / Aggressive
    juce::ImageComponent chordColorLbl;
    juce::ImageButton  rhythmicVoicingToggle;
    juce::ImageComponent rhythmicVoicingLbl;
    ...
    EngineSel engineSel{ EngineSel::Melody };    // passed into generator
