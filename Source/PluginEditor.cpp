...
// --- Center HUMANIZE label image above the humanize sliders ---
    juce::Rectangle<int> humanizeArea = timingSl.getBounds();
    yHumanize = timingSl.getY();
    humanizeArea = humanizeArea.getUnion(velocitySl.getBounds());
    humanizeArea = humanizeArea.getUnion(swingSl.getBounds());
    humanizeArea = humanizeArea.getUnion(feelSl.getBounds());
