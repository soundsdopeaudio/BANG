...
// ---- Top row: soundsDope logo (left), Title image (center), Dice (right) ----
    ...
    // bounds.removeFromTop(rowGap - 4); // This is now handled cleanly by topUsed
    {
        ...
        const int advBtnW = 190;
        const int advBtnH = 150; // Taller as in the mockup
...
// --- CHORDS ENGINE ---
{
    // Collect the same settings you already pass to the other modes:
    {
        ...
        gen.setHumanizeTiming(static_cast<float>(timingSl.getValue()));
        gen.setHumanizeVelocity(static_cast<float>(velocitySl.getValue()));
        gen.setSwingAmount(static_cast<float>(swingSl.getValue()));
        gen.setFeelAmount(static_cast<float>(feelSl.getValue()));
    ...
    struct ReharmonizeComponent : public juce::Component
    {
        ...
        ReharmonizeComponent(BANGAudioProcessorEditor& ed) : editor(ed)
        {
            ...
            regenerateBtn.onClick = [this]
            {
                ...
                // 1) Push popup selections to the generator (key/scale come from this popup)
                gen.setKey(rootBoxToSemitone(keyBox) + 60);        // absolute root at C4-based MIDI
                gen.setScaleIndex(scaleBox.getSelectedId());

                // Keep the main editor’s meter/bars
                const int beatsPerBar = editor.currentTSNumerator();
                const int barsTotal = editor.currentBars();         // expected 8
                gen.setTimeSignature(beatsPerBar, 4);
                gen.setBars(barsTotal);

                // Keep humanize/rest/engine/etc. synced from the main UI
                editor.pushSettingsToGenerator();

                // 2) Generate a fresh idea in the new key/scale
                auto bundle = gen.generateMelodyAndChords(/*avoidOverlaps*/ true);

                const bool keepFirstHalf = keepBarsToggle.getToggleState();
                if (keepFirstHalf && barsTotal >= 2)
                {
                    // ---- HALF/HALF MERGE WITH TRIMMING AT THE BOUNDARY ----
                    const double splitBeat = (barsTotal / 2.0) * (double)beatsPerBar;
                    const double totalBeats = (double)barsTotal * (double)beatsPerBar;

                    const auto& oldMel = editor.getCachedMelody();
                    const auto& oldChd = editor.getCachedChords();

                    auto trimRightAt = [](const Note& n, double cutBeat) -> Note
                    {
                        Note t = n;
                        const double endBeat = n.startBeats + n.lengthBeats;
                        if (endBeat > cutBeat)
                            t.lengthBeats = std::max(0.0, cutBeat - n.startBeats);
                        return t;
                    };

                    auto trimLeftFrom = [](const Note& n, double cutBeat) -> Note
                    {
                        Note t = n;
                        const double endBeat = n.startBeats + n.lengthBeats;
                        if (n.startBeats < cutBeat && endBeat > cutBeat)
                        {
                            t.lengthBeats = std::max(0.0, endBeat - cutBeat);
                            t.startBeats = cutBeat;
                        }
                        return t;
                    };

                    auto mergeHalf = [&](const std::vector<Note>& oldVec,
                        const std::vector<Note>& newVec) -> std::vector<Note>
                    {
                        std::vector<Note> out;
                        out.reserve(oldVec.size() + newVec.size());

                        // Keep LEFT half from OLD:
                        for (const auto& n : oldVec)
                        {
                            ...
                            {
                                // crosses the split → keep trimmed to the left side
                                Note trimmed = trimRightAt(n, splitBeat);
                                if (trimmed.lengthBeats > 0.0)
                                    out.push_back(trimmed);
                            }
                            // else: entirely in right half → drop (we’ll replace with NEW)
                        }

                        // Take RIGHT half from NEW:
                        for (const auto& n : newVec)
                        {
                            const double nStart = n.startBeats;
                            const double nEnd = n.startBeats + n.lengthBeats;

                            if (nStart >= splitBeat && nStart < totalBeats)
                            {
                                // normal case: starts inside the right half
                                out.push_back(n);
                            }
                            else if (nStart < splitBeat && nEnd > splitBeat)
                            {
                                // if any new note happens to cross from left → trim to the right side
                    ...
                    // Merge both tracks with trimming
                    auto mergedMel = mergeHalf(oldMel, bundle.melody);
                    auto mergedChd = mergeHalf(oldChd, bundle.chords);
                ...
                {
                    // Replace all bars with the new reharm
                    editor.setCachedNotesAndRefresh(std::move(bundle.melody), std::move(bundle.chords));
                }
...
void BANGAudioProcessorEditor::openAdjust()
{
    struct AdjustWindow : public juce::Component
    {
        ...
        void resized() override
        {
            ...
            auto content = r.withWidth(r.getWidth() * int(0.8f)).withX(r.getCentreX() - (r.getWidth() * int(0.8f)) / 2);
            ...
            // ---- Toggles with larger labels ----
            {
                auto toggleWidthForHeight = [](int h) -> int
                {
                    ...
                    return juce::roundToInt((static_cast<float>(baseW) / static_cast<float>(baseH)) * static_cast<float>(h));
                };
