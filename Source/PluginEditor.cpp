...
// Make the editor resizable again
    ...
    setResizeLimits(/*minW*/ 1200, /*minH*/ 720,
        /*maxW*/ 3200, /*maxH*/ 2000);
...
// Load images from BinaryData
   ...
   // addAndMakeVisible(timingLbl);   timingLbl.setText("HUMANIZE TIMING", juce::dontSendNotification);
    ...
    pianoRoll.setVerticalZoom(1.15f);
    ...
    resized(); // <-- run layout now that rollView/pianoRoll are set up
}
...
void BANGAudioProcessorEditor::resized()
{
    ...
    const int margin = 8;
    // ---- Row 4: Piano roll ----
    rollView.setBounds(bounds);   // bounds = the remaining central area after your other rows
    updateRollContentSize();      // this sets pianoRoll size based on bars/TS/pxPerBeat
...
void BANGAudioProcessorEditor::updateRollContentSize()
{
    ...
    if (vpH <= 0)
        vpH = getHeight() - 60 /* top rows */ - 52 /* harmony row */ - 70 /* bottom row */ - 32 /* margins */;
