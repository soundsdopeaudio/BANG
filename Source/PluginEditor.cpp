...
// Load images from BinaryData
   ...
   // addAndMakeVisible(timingLbl);   timingLbl.setText("HUMANIZE TIMING", juce::dontSendNotification);
    ...
    rollView.setScrollBarsShown(true, false);          // <-- horizontal + vertical
#if JUCE_MAJOR_VERSION >= 8
    rollView.setScrollOnDragMode(juce::Viewport::ScrollOnDragMode::nonHover);
#else
...
void BANGAudioProcessorEditor::openPolyrhythm()
{
    ...
    struct PolyComp : public juce::Component
    {
        ...
        PolyComp(BANGAudioProcessorEditor& ed) : editor(ed)
        {
            ...
            // use your look-and-feel from the editor
        ...
        void resized() override
        {
            ...
            const auto toggleWFor = [](int h)
            {
                ...
                if (baseW == 0 || baseH == 0)
                {
                    ...
                    else { baseW = 352; baseH = 125; }
                }
                return juce::roundToInt((baseW / (float)baseH) * (float)h);
            };

            const int tH = 40;                       // 40–48 looks good here
            const int tW = toggleWFor(tH);

            typesBox.items.add(juce::FlexItem(twentyThreeLbl).withWidth(74).withHeight(43));
            typesBox.items.add(juce::FlexItem(poly23Btn).withWidth(44).withHeight(44));
            typesBox.items.add(juce::FlexItem(thirtyTwoLbl).withWidth(74).withHeight(43));
            typesBox.items.add(juce::FlexItem(poly32Btn).withWidth(44).withHeight(44));
            typesBox.items.add(juce::FlexItem(thirtyFourLbl).withWidth(74).withHeight(43));
            typesBox.items.add(juce::FlexItem(poly34Btn).withWidth(44).withHeight(44));
            typesBox.items.add(juce::FlexItem(fortyThreeLbl).withWidth(74).withHeight(43));
            typesBox.items.add(juce::FlexItem(poly43Btn).withWidth(44).withHeight(44));
...
void BANGAudioProcessorEditor::openReharmonize()
{
    ...
    struct ReharmonizeComponent : public juce::Component
    {
        ...
        void resized() override
        {
            ...
            const int toggleW = juce::roundToInt((352.0f / 125.0f) * toggleH);
            keepFirst4BarsLbl.setBounds(row.removeFromLeft(row.getWidth() / 2).reduced(5));

            bounds.removeFromTop(40);

            keepBarsToggle.setBounds(row.removeFromRight(toggleW).withHeight(toggleH));

            juce::Rectangle<int> newBounds(10, 20, 200, 61);
