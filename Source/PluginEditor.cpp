...
void BANGAudioProcessorEditor::resized()
{
...

    {
        ...
        // Layout constants from the header
        const int size = kIconSize;
        const int pad = kIconPadding;

        // Top-left origin (pad, pad)
        Rectangle<int> area = getLocalBounds().removeFromTop(size + 2 * pad).removeFromLeft((size * 2) + (pad * 3));
        area.reduce(pad, pad); // inner padding

        // Place Undo then Redo horizontally
        auto left = area.removeFromLeft(size);
        left.setHeight(size);
        undoButton.setBounds(left);

        area.removeFromLeft(pad);

        auto right = area.removeFromLeft(size);
        right.setHeight(size);
        redoButton.setBounds(right);
        area.removeFromBottom(10);
    }

    // ---- Top right dice button ----
    diceBtn.setBounds(bounds.getRight() - 44 - pad, bounds.getY(), 44, 44);
    ...
    auto harmonyRow = bounds.removeFromTop(52);
    {
        ...
        const int btnH = 44;
        const int btnGap = 20;
        const int totalWidth = btnW * 3 + btnGap * 2;

        // Center the whole row of buttons
        auto buttonArea = harmonyRow.withSizeKeepingCentre(totalWidth, btnH);

        reharmBtn.setBounds(buttonArea.removeFromLeft(btnW));
        buttonArea.removeFromLeft(btnGap);
        advancedBtn.setBounds(buttonArea.removeFromLeft(btnW));
        buttonArea.removeFromLeft(btnGap);
        adjustBtn.setBounds(buttonArea.removeFromLeft(btnW));
    }

    // ---- Row 5 (Bottom): Generate and Drag buttons ----
    auto bottomArea = bounds.removeFromBottom(90);
    {
        const juce::Rectangle<int> maxBounds(0, 0, 500, 150); // Increased size
        const int bigGap = 30;

        float genNatW = generateBtnNaturalBounds.getWidth();
        float genNatH = generateBtnNaturalBounds.getHeight();
        if (genNatW == 0) genNatW = 1; if (genNatH == 0) genNatH = 1;
        ...
        if (dragNatW == 0) dragNatW = 1; if (dragNatH == 0) dragNatH = 1;
        ...
        generateBtn.setBounds(bigButtonRow.removeFromLeft(genScaledW));
        bigButtonRow.removeFromLeft(bigGap);
        dragBtn.setBounds(bigButtonRow);
    }
