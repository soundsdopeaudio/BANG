...
void BANGAudioProcessorEditor::resized()
{
    ...
    const int pad = 16;
    const int rowGap = 16;
    const int controlColWidth = 280; // Width for left (combos) and right (sliders) columns
    ...
    auto mainControlsRow = bounds.removeFromTop(260);
    ...
    {
        const int labelImgW = 92;
        const int labelImgH = 36;
        const int comboW = 160;
        const int comboH = 28;
        const int itemGap = 12;
        
        auto comboArea = leftCol.withTrimmedTop(20);
    ...
    // ---- Center Logo ----
    {
        ...
        {
            logoH = juce::roundToInt(logoW / img.getAspectRatio());
        }
    ...
    auto harmonyRow = bounds.removeFromTop(52);
    {
        ...
        fb.items.add(juce::FlexItem(reharmBtn).withFlex(1.0f).withMaxHeight(44));
        fb.items.add(juce::FlexItem(advancedBtn).withFlex(1.0f).withMaxHeight(44));
        fb.items.add(juce::FlexItem(adjustBtn).withFlex(1.0f).withMaxHeight(44));
        
        fb.performLayout(harmonyRow.reduced(20, 0));
    }

    // ---- Row 5 (Bottom): Generate and Drag buttons ----
    auto bottomArea = bounds.removeFromBottom(90);
    {
        const juce::Rectangle<int> maxBounds(0, 0, 300, 75); // Increased size
        const int bigGap = 30;
