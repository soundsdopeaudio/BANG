...
void AdvancedHarmonyWindow::resized()
{
    ...
    auto extensionsArea = bounds.removeFromTop(checkboxHeight);
    int x = horizontalMargin;
    auto addExtension = [&](juce::ImageComponent& label, juce::ToggleButton& checkbox)
    {
        label.setBounds(x, extensionsArea.getY(), 50, checkboxHeight);
        x += 50 + labelCheckboxGap;
        checkbox.setBounds(x, extensionsArea.getY(), checkboxWidth, checkboxHeight);
        x += checkboxWidth + controlGroupGap;
    };

    addExtension(label7ths, checkbox7ths);
    addExtension(label9ths, checkbox9ths);
    addExtension(label11ths, checkbox11ths);
    addExtension(label13ths, checkbox13ths);
    addExtension(labelSus, checkboxSus);
    addExtension(labelAlt, checkboxAlt);
    addExtension(labelSlash, checkboxSlash);
    ...
    auto addAdvancedRow = [&](juce::Component& label, juce::ToggleButton& checkbox, juce::ImageComponent& densityLabel, juce::Slider& slider)
    {
        ...
        checkbox.setBounds(rowArea.getX() + horizontalMargin + 150 + labelCheckboxGap, rowArea.getY(), checkboxWidth, checkboxHeight);
