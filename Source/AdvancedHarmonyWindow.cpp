...
void AdvancedHarmonyWindow::setImageButton3(juce::ImageButton& btn, const juce::String& baseHint)
{
    ...
    auto o = loadImageByHint(baseHint + "_hover");      // e.g. diceBtn_over.png
...
AdvancedHarmonyWindow::AdvSnapshot AdvancedHarmonyWindow::captureFromApvts() const
{
    auto& apvts = processor.apvts;
    AdvSnapshot s;

    auto LB = [&](const char* id)->bool {
        if (auto* rp = apvts.getRawParameterValue(id)) return rp->load() > 0.5f;
        return false;
    };
    auto LF01to100 = [&](const char* id)->float {
        if (auto* p = apvts.getParameter(id))
            if (auto* pf = dynamic_cast<juce::AudioParameterFloat*>(p))
                return pf->get(); // returns the real, user-facing value (already in your [min..max] range)
        return 25.0f;
    };
    ...
    s.alt = LB("alt");
    s.slash = LB("slash");
    s.extDensity01to100 = LF01to100("extDensity");

    // Advanced families
    s.advSecDom = LB("advSecDom");
    s.advBorrowed = LB("advBorrowed");
    s.advChromMed = LB("advChromMed");
    s.advNeapolitan = LB("advNeapolitan");
    s.advTritone = LB("advTritone");

    return s;
}

void AdvancedHarmonyWindow::applyToApvts(const AdvSnapshot& s)
{
    // Use your existing APVTS setters so hosts get change gestures
    setBoolParam("ext7", s.ext7);
    setBoolParam("ext9", s.ext9);
    setBoolParam("ext11", s.ext11);
    setBoolParam("ext13", s.ext13);
    setBoolParam("sus24", s.sus24);
    setBoolParam("alt", s.alt);
    setBoolParam("slash", s.slash);
    setFloatParam01to100("extDensity", s.extDensity01to100);

    setBoolParam("advSecDom", s.advSecDom);
    setBoolParam("advBorrowed", s.advBorrowed);
    setBoolParam("advChromMed", s.advChromMed);
    setBoolParam("advNeapolitan", s.advNeapolitan);
    setBoolParam("advTritone", s.advTritone);
}
    ...
    : processor(p)
{
    ...
    enablePassBtn.onClick = [this]
    {
        ...
        // The user explicitly accepted the changes; keep them.
        committed_ = true;
    ...
    densitySlider.setLookAndFeel(&densitySliderLAF);
    ...
    originalSnapshot_ = captureFromApvts();

    densitySlider.setTooltip("Adjusts the Extensions/Other Chords' section's influence on the midi generated.");
    enablePassBtn.setTooltip("Enables the Advanced Harmony window to influence the midi generated and closes the window.");
}

AdvancedHarmonyWindow::~AdvancedHarmonyWindow()
{
    // If the user did NOT press Enable, revert any changes the attachments made
    if (!committed_)
    {
        applyToApvts(originalSnapshot_);
    }
    else
    {
        // User committed the advanced changes — make absolutely sure the processor
        // refreshes its shadow copy and the generator gets the updated pointer.
        //
        // This guarantees the generator will see the newly-enabled families/extensions
        // immediately after the Advanced window is accepted.
        processor.refreshAdvancedOptionsFromApvts();
        processor.getMidiGenerator().setAdvancedHarmonyOptions(processor.getAdvancedOptionsPtr());
    }
}
...
void AdvancedHarmonyWindow::resized()
{
    ...
    // Adjust 'advancedChordsLbl' (advImg) to align with the controls below it.
    auto advImgRow = r.removeFromTop(40);
    advImgRow.removeFromRight(100); // Match alignment of labels below by removing the same amount of space.
    advImg.setBounds(advImgRow.withSizeKeepingCentre(getWidth(advImg, 30), 30));
    ...
    int passH = 150;
    enablePassBtn.setBounds(r.withSizeKeepingCentre((getButtonWidth(enablePassBtn, passH) * 2), (passH * 2)));
}
