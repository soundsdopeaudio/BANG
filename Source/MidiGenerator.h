...
{
    juce::String      name;
    std::vector<int>  intervals;   // semitone steps from tonic, 0..11
};
...
public:
    ...
    // ===== Scales / Key / Meter ============================================
    static const std::vector<Scale>& getKnownScales();
    ...
    enum class PolyrhythmMode { None, Ratio3_2, Ratio4_3, Ratio5_4, Ratio7_4 };
    ...
    const Scale& getScaleByIndex(int idx) const;
    const Scale& getScaleByName(const juce::String& name) const;
    extern const std::vector<Scale> kScales; // if you declare it in the header
