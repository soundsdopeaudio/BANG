...
public:
    ...
    // New: Call & Response mode (Capture a motif and generate a musical reply phrase)
    void setCallAndResponseEnabled(bool enable) noexcept { callAndResponseEnabled_ = enable; }
    bool isCallAndResponseEnabled() const noexcept { return callAndResponseEnabled_; }
    void applyCallAndResponse(std::vector<Note>& out, int bars, int beatsPerBar, int keyPc) const;


    // --- Motif Mode: enable looped motif repetition/variation
    void setMotifModeEnabled(bool enable) noexcept { motifModeEnabled_ = enable; }
    bool isMotifModeEnabled() const noexcept { return motifModeEnabled_; }


    // Harmony stacks mode enum and API (if not already)
    enum class HarmonyStackMode { Off = 0, Third = 1, Sixth = 2, OpenFifth = 3, Spread = 4 };
    void setHarmonyStackMode(HarmonyStackMode m) noexcept;
    HarmonyStackMode getHarmonyStackMode() const noexcept;

    // Counter
    void setCounterEnabled(bool enable) noexcept;
    bool isCounterEnabled() const noexcept;

    // Builders
    std::vector<Note> makeCallAndResponse(const std::vector<Note>& src, int bars, int beatsPerBar, int keyPc) const;
    std::vector<Note> makeHarmonyStack(const std::vector<Note>& melody) const;
    std::vector<Note> makeCounterMelody(const std::vector<Note>& melody) const;

    // Chord Color (tension) modes
    enum class ChordColorMode { ColorOff = 0, ColorLight = 1, ColorModerate = 2, ColorAggressive = 3 };

    // API to control the modes
    void setChordColorMode(ChordColorMode m) noexcept;
    ChordColorMode getChordColorMode() const noexcept;

    void setRhythmicVoicingEnabled(bool e) noexcept;
    bool isRhythmicVoicingEnabled() const noexcept;

    // Processing helpers (public so editor/tests can call if needed)
    void applyChordColorTensions(std::vector<Note>& chordNotes, int rootPC, const std::vector<int>& scaleIntervals, int tsNum, float feelAmt) const;
    void applyRhythmicVoicing(std::vector<Note>& chordNotes, int tsNum, double gridUnit) const;


    // Rhythm feature toggles
    void setAllowDotted(bool b) noexcept { allowDotted_ = b; }
    void setAllowTriplets(bool b) noexcept { allowTriplets_ = b; }
    ...
    bool getAllowDotted()   const noexcept { return allowDotted_; }
    bool getAllowTriplets() const noexcept { return allowTriplets_; }
...
private:
    ...
    // --- new internal flags ---
    bool callAndResponseEnabled_ = false;

    // Motif flag (controls whether bang_applyLoopingPhrases runs)
    bool motifModeEnabled_ = true; // default ON (you can change default if you like)

    bool counterEnabled_ = false;
    HarmonyStackMode harmonyStackMode_ = HarmonyStackMode::Off;
    // chord-engine specific state
    ChordColorMode chordColorMode_ = ChordColorMode::ColorOff;
    bool rhythmicVoicingEnabled_ = false;
    ...
    // --- Predominant note length bias for melody generation ---------------
    enum class PredLen { Sixteenth, Eighth, Quarter, Half, Triplet8th };
    PredLen predominantLen_ = PredLen::Eighth; // default; randomized per generation
    ...
    bool allowDotted_ = true;  // enable dotted durations (e.g., 0.75, 1.5, 3.0 beats)
    bool allowTriplets_ = true;  // enable triplet durations (e.g., 1/3, 2/3, 4/3 beats)
