...
{
    // --- Extensions / Other ---
    advancedOptsShadow.ext7 = apvts.getRawParameterValue("ext7")->load() > 0.5f;
    advancedOptsShadow.ext9 = apvts.getRawParameterValue("ext9")->load() > 0.5f;
    advancedOptsShadow.ext11 = apvts.getRawParameterValue("ext11")->load() > 0.5f;
    advancedOptsShadow.ext13 = apvts.getRawParameterValue("ext13")->load() > 0.5f;
    advancedOptsShadow.sus2 = apvts.getRawParameterValue("sus24")->load() > 0.5f; // treat as sus2/4 together
    advancedOptsShadow.sus4 = advancedOptsShadow.sus2; // keep them tied in this UI
    advancedOptsShadow.altered = apvts.getRawParameterValue("alt")->load() > 0.5f;
    advancedOptsShadow.slash = apvts.getRawParameterValue("slash")->load() > 0.5f;

    const float densityPct = apvts.getRawParameterValue("extDensity")->load();
    advancedOptsShadow.extDensity = juce::jlimit(0.0f, 100.0f, densityPct);

    // --- Advanced chords ---
    advancedOptsShadow.secondaryDominants = apvts.getRawParameterValue("advSecDom")->load() > 0.5f;
    advancedOptsShadow.borrowedChords = apvts.getRawParameterValue("advBorrowed")->load() > 0.5f;
    advancedOptsShadow.chromaticMediants = apvts.getRawParameterValue("advChromMed")->load() > 0.5f;
    advancedOptsShadow.neapolitan = apvts.getRawParameterValue("advNeapolitan")->load() > 0.5f;
    advancedOptsShadow.tritoneSubs = apvts.getRawParameterValue("advTritone")->load() > 0.5f;
}
